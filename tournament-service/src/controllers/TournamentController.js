...
const setFixtures = async (req, res) => {
    try {
        ...
        // Validate request
        if (!tournamentId || !tournamentFormat || !startDate) {
            return res.status(400).json(reply.failure("Missing required fixture parameters."));
        }
        ...
        let roundNumber = round ? round.roundNumber + 1 : 1;

        let fixtureTeams = [];

        if (!round) {
            fixtureTeams = teams.map(({ teamID }) => ({
                id: teamID._id,
        ...
        // Initialize fixture generator controller
        ...
        await tournamentController.generateFixtures();
    } catch (error) {
